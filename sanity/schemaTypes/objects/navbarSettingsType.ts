...
export const navbarSettingsType = defineType({
  ...
  preview: {
    select: {
      linkType: "homeLink.linkType",
      internalPageTitle: "homeLink.internalPage.title",
      externalUrl: "homeLink.externalUrl",
      navigationItems: "navigationItems",
    },
    prepare(selection) {
      const { linkType, internalPageTitle, externalUrl, navigationItems } = selection;
      const title = linkType === "internal" ? internalPageTitle || "Home (internal)" : externalUrl || "Home (external)";
      const subtitle = `${(navigationItems || []).length} navigation item(s)`;
